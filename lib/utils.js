...
// Converts a raw 24-hour string (e.g., "09:30" or "18:30") to AM/PM
...
export function calcTotalBreakMs(breaks, nowMs = Date.now()) {
...
export function calcGrossSessionDurationMs(session, nowMs = Date.now()) {
...
export function calcSessionDurationMs(session, breaks = [], nowMs = Date.now()) {
  if (!session?.punch_in_time) return 0;
  const start = new Date(session.punch_in_time).getTime();
  const end = session.punch_out_time ? new Date(session.punch_out_time).getTime() : nowMs;
